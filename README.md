...
把音乐客户端下载的加密音频（`.mflac` / `.mgg`）一键转换成通用的 FLAC / MP3。

TuneLift 通过注入正在运行的音乐客户端完成解密，再调用 ffmpeg 转码。提供图形界面和命令行两种用法，
...
- Windows 10 / 11（64 位）
- **必须提前启动音乐客户端（QQ音乐电脑版）并保持运行** —— 解密依赖它的进程
...
本工具只做**格式转换**：把音乐客户端**已经能为你播放**的加密音频转成通用格式。
它不提供账号、不绕过登录，也拿不到你没有权限播放的内容。

需要自行承担的风险：

- **账号** —— 绕过技术措施违反音乐平台的用户协议，平台有权直接封号，
  **VIP 身份不构成豁免**。
...
2. **解密** —— 注入音乐客户端进程，把加密音频解密成 FLAC / OGG
...
退出码：`0` 全部成功；`2` 环境或参数不满足（输入路径不存在、给的不是加密音频、缺 `ffmpeg.exe`、输出目录不可用、音乐客户端未运行、解密脚本未生效）；`3` 有文件解密失败；`4` 有文件转码失败。方便在批处理里判断成败。
...
先启动音乐客户端并保持运行。如果已经启动仍报错，试试以管理员身份运行。

**提示「hook 脚本没有生效」**

客户端开着，但还没加载 `QQMusicCommon.dll`。在客户端里随便播放一首歌再重新运行本程序即可。

**解密失败，提示 `invalid size` 或类似错误**

可能是客户端更新了加密算法，内置的 `hook_qq_music.js` 不再兼容。
...
程序使用了 frida 动态注入技术（用于与音乐客户端进程通信），部分杀毒软件会误报。把 `TuneLift.exe` 加入信任区即可，或临时关闭杀毒软件后再运行。
...
- **Frida** —— 动态代码注入框架，解密靠它把钩子脚本注入音乐客户端进程
