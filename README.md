...
它做的是**格式转换**：把 QQ音乐客户端**已经能为你播放**的加密音频（`.mflac` / `.mgg`）
转成通用的 FLAC / MP3。

它**不提供账号、不绕过登录、也拿不到你没有权限播放的内容**——你买过什么，
它才能处理什么。如果你在客户端里播放不了某首歌，这个工具同样处理不了它。

### 账号风险

- 绕过客户端的技术措施**违反 QQ音乐的用户协议**。平台有权单方面处置，
  **后果通常就是账号被封**，不需要经过司法程序。
- **VIP 身份不构成豁免。** 购买记录会影响赔偿数额的认定，但不改变行为本身的定性；
  反过来说，实名且有支付记录的账号，真出问题时更容易定位到具体的人。
- 本工具通过注入 `QQMusic.exe` 进程工作。解密过程本身不联网（本项目不含任何网络请求，
  可自行检索源码确认），但**客户端自己可能记录行为**，恢复联网后上报。
- 客户端更新后可能检测到这类注入。

### 版权风险

- 依据《著作权法》第五十三条，故意避开或破坏技术措施属于侵权行为。
- **分水岭是「有没有传出去」**：仅自己离线收听，实际被追究的可能性极低；
  一旦上传网盘、分享给他人或在公开渠道发布，就同时涉及信息网络传播权，
  性质完全不同。
- 建议保留下载与购买凭证——万一有争议，这是「合法来源」的证据。

### 免责声明

本项目仅供学习与技术研究使用。使用者应当确保：

1. 只处理**自己合法购买、且自己有权访问**的音频文件；
2. **不传播、不分享、不再分发**处理后的文件；
3. **不用于任何商业用途**；
4. 遵守所在地区的法律法规。

因使用本工具产生的一切后果由使用者自行承担，作者不承担任何责任。
如果版权所有者或相关平台认为本项目不当，请通过 issue 联系，我们会及时处理。
